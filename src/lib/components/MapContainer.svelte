	...
	onMount(async () => {
		try {
			...
			const unsubZoneMap = mobileZoneMapActive.subscribe((active) => {
				...
				if (active && mapService.map?.getBounds) {
					const bounds = mapService.map.getBounds();
					if (bounds) {
						const sw = bounds.getSouthWest();
						const ne = bounds.getNorthEast();
						h3Actions.setResolutionForMobile(Math.abs(ne.lat() - sw.lat()));
					}
				}
			});
			h3GridOverlayService.setOnCellsRendered((count) => h3Actions.setRenderedCells(count));

			const syncResolutionFromViewport = () => {
				if (!get(mobileZoneMapActive) || !mapService.map?.getBounds) return;
				const bounds = mapService.map.getBounds();
				if (!bounds) return;
				const sw = bounds.getSouthWest();
				const ne = bounds.getNorthEast();
			...
			const zoomListener = map.addListener('zoom_changed', syncResolutionFromViewport);
			const idleResolutionListener = map.addListener('idle', syncResolutionFromViewport);
