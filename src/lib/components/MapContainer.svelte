<script>
	...
	import { showH3Grid, h3Resolution, selectedH3Cells, h3Actions } from '$lib/stores/h3Store.js';
	import {
...
export let onMapTap = () => {};
	...
	let mapElement;
	let map;
	let showGeofenceMenu = false;
	let showAlertsPanel = false;

	// ── estado local del menú ──────────────────────────
	let editingName = false; // muestra input de nombre
	let colorInputEl; // ref al <input type="color"> oculto
	let geofenceNameInputEl;

	const GF_TYPES = [
		{ id: 'polygon', label: 'Polígono', icon: 'mdi:vector-polygon' },
		{ id: 'polyline', label: 'Ruta', icon: 'mdi:vector-polyline' },
		{ id: 'corridor', label: 'Corredor', icon: 'mdi:road-variant' },
		{ id: 'circle', label: 'Círculo', icon: 'mdi:circle-outline' },
		{ id: 'rectangle', label: 'Rectángulo', icon: 'mdi:rectangle-outline' },
		{ id: 'marker', label: 'Marcador', icon: 'mdi:map-marker' }
	];

	// Cuando se cierra el menú, resetear nombre local
	$: if (!showGeofenceMenu) {
		editingName = false;
	}

	$: if (editingName) {
		tick().then(() => geofenceNameInputEl?.focus());
	}

	function startGeofenceType(type) {
		if (isLoading || !mapService.map || !mapService.google) return;
		geofenceActions.setActiveType(type);
		if (!geofenceService.startDrawing(type)) return;
	...
	function toggleMenu() {
		...
		if ($isDrawingGeofence) {
			...
			showGeofenceMenu = !showGeofenceMenu;
			if (!showGeofenceMenu) editingName = false;
		}
	}
	...
	function handleKeydown(e) {
		if (e.key === 'Escape') {
			...
			if ($isDrawingGeofence) cancelDrawing();
		}
	}
	...
	/** delta +1 = hexágonos más pequeños (mayor resolución H3); -1 = más grandes */
	function stepH3Resolution(delta) {
		const cur = get(h3Resolution);
		const next = Math.min(H3_RES_MAX, Math.max(H3_RES_MIN, cur + delta));
		if (next !== cur) h3Actions.setResolution(next);
	}
	...
	onMount(async () => {
		try {
			...
			h3GridOverlayService.setOnCellSelect((h3Index) => h3Actions.toggleCell(h3Index));
			h3GridOverlayService.setOnCellsRendered((count) => h3Actions.setRenderedCells(count));
			geofenceService.attachMap(mapService.map, mapService.google);
			geofenceService.setDrawingManagerColor(get(pendingGeofenceColor));
			geofenceService.setOnPending((draft) =>
				geofenceActions.openDraft({ mode: 'create', ...draft })
			);
			geofenceService.setOnCreated((geofence) => {
				geofenceActions.addGeofence(geofence);
				geofenceActions.stopDrawing();
			});
			geofenceService.setOnOverlayClick((id) => {
				const g = get(geofences).find((x) => x.id === id);
				if (!g) return;
				if (get(isDrawingGeofence)) {
					geofenceService.stopDrawing();
					geofenceActions.stopDrawing();
				}
				const draft = get(geofenceDraft);
			...
			const unsubDrawColor = pendingGeofenceColor.subscribe((c) =>
				geofenceService.setDrawingManagerColor(c)
			);
...
<section
	...
	data-theme={$theme}
	aria-label="Mapa de seguimiento y geocercas"
>
	{#if isLoading}
		<div
	...
	<!-- ── Controles de geocerca (solo con mapa listo: evita modo “dibujando” sin listeners) ── -->
					...
					>
						...
						<div class="max-h-[320px] overflow-y-auto p-3 text-white/85">
							...
							{#if $alerts.length === 0 && $alarmEvents.length === 0}
								<p class="m-0 rounded-lg border border-white/[0.08] bg-white/[0.03] p-2 text-xs text-white/50">
						...
						<div class="min-w-0 flex-1">
							{#if editingName}
								<input
									...
									on:input={(e) =>
										geofenceActions.setPendingGeofenceName(e.currentTarget.value)}
	...
	<!-- ── Control H3 selección: mitad de la pantalla, alineado a la derecha ── -->
	{#if $showH3Grid}
		<div
