<script>
	...
	import { geofences } from '$lib/stores/geofenceStore.js';
	...
	$: configuredGeofenceZones = $geofences.map((gf) => ({
		id: gf.id,
		name: gf.name || 'Geocerca sin nombre',
	...
	$: filteredH3Zones = normalizedCellQuery
		...
		: allH3Zones;
	$: allZoneOptions = [...configuredGeofenceZones, ...allH3Zones];
...
{#if wizard}
	<div
		class="fixed inset-0 z-[150] flex flex-col bg-[#060b18] font-sans text-white antialiased"
		role="dialog"
		aria-modal="true"
		aria-labelledby="caw-title"
	>
		<!-- Progress bar + header -->
		<div class="shrink-0 border-b border-white/[0.07] bg-[#060b18]/95 backdrop-blur-sm">
			<!-- Progress bar -->
			...
			<header class="flex items-center gap-3 px-4 py-3">
				<button
					type="button"
					class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-white/[0.06] text-white/60 transition-colors hover:bg-white/[0.1] hover:text-white focus-visible:outline-2 focus-visible:outline-sky-400"
					on:click={close}
				...
				<div class="min-w-0 flex-1">
					<h2 id="caw-title" class="m-0 text-[15px] font-bold tracking-tight text-white">
						Crear alerta
					</h2>
					<p class="m-0 text-[10px] text-white/35">Paso {displayStep} de {effectiveSteps}</p>
				</div>
		...
		<main class="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 py-5">
			...
			{#if step === 1}
				...
				<p class="mb-5 text-[12px] text-white/40 leading-relaxed">
				...
				<div class="flex flex-col gap-3">
					...
					<button
						...
						{wizard.type === 'ignition'
							...
							: 'border-transparent bg-white/[0.05] hover:border-white/[0.12] hover:bg-white/[0.08]'}"
					...
					>
						...
						<div class="min-w-0 flex-1">
							...
							<p class="m-0 mt-1 text-[12px] text-white/45 leading-snug">
			...
			{:else if step === 2}
				...
				<p class="mb-5 text-[12px] text-white/40">Elige la condición que disparará la alerta.</p>
						...
						{#each [{ value: 'on', label: 'Encendido', icon: 'mdi:lightning-bolt', color: 'blue' }, { value: 'off', label: 'Apagado', icon: 'mdi:engine-off-outline', color: 'slate' }] as opt}
							<button
								...
								{wizard.condition === opt.value
									...
									: 'border-transparent bg-white/[0.06] hover:border-white/[0.14]'}"
			...
			{:else if step === 3}
				...
				<p class="mb-4 text-[12px] text-white/40 leading-relaxed">
				...
				{#if $vehicles.length === 0}
					<p class="py-8 text-center text-[13px] text-white/38">No hay unidades disponibles.</p>
				{:else}
						...
						{#each $vehicles as vehicle}
							...
							<button
								...
								{selected
									...
									: 'border-transparent bg-white/[0.05] hover:border-white/[0.1]'}"
			...
			{:else if step === 4 && wizard.type === 'zone'}
				...
				<p class="mb-4 text-[12px] text-white/40">
					Elige la zona H3 que define el perímetro de la alerta.
				</p>
				{#if configuredGeofenceZones.length === 0 && allH3Zones.length === 0}
					<div class="rounded-2xl border border-white/[0.07] bg-white/[0.03] p-6 text-center">
						<Icon
							...
							aria-hidden="true"
						/>
						<p class="m-0 text-[13px] font-semibold text-white/60">Sin zonas configuradas</p>
						<p class="m-0 mt-1 text-[11px] text-white/35">
							Crea una geocerca o selecciona celdas H3 en el mapa para usarlas aquí.
						</p>
					</div>
				{:else}
					<div class="flex flex-col gap-2">
						{#if configuredGeofenceZones.length > 0}
							<p class="m-0 mb-1 mt-1 text-[10px] font-bold uppercase tracking-widest text-white/35">
								Geocercas
							</p>
							{#each configuredGeofenceZones as zone}
								<button
									type="button"
									class="flex items-center gap-3 rounded-[14px] border-2 px-3.5 py-3 text-left transition-all duration-150 focus-visible:outline-2 focus-visible:outline-sky-400
									{wizard.zone === zone.id
								...
								>
									...
									<div class="min-w-0 flex-1">
										...
										<p class="m-0 mt-0.5 text-[11px] text-white/40">Geocerca {zone.shape}</p>
									</div>
						...
						{#if allH3Zones.length > 0}
							<p class="m-0 mb-1 mt-3 text-[10px] font-bold uppercase tracking-widest text-white/35">
								Zonas H3 (por celda ID)
							</p>
							<input
								type="text"
								class="w-full rounded-[12px] border border-white/[0.14] bg-white/[0.06] px-3 py-2 text-[12px] text-white outline-none placeholder:text-white/28 focus:border-blue-500/60 focus:ring-[3px] focus:ring-blue-500/15"
							...
							{#if filteredH3Zones.length === 0}
								<p class="m-0 rounded-xl border border-white/[0.08] bg-white/[0.03] px-3 py-2 text-[11px] text-white/45">
									...
									>
										...
										<div class="min-w-0 flex-1">
											...
											{#if zone.cells.length > 0}
												<p class="m-0 mt-1 truncate font-mono text-[10px] text-white/30">
			...
			{:else if step === 5}
				...
				<label for="caw-name" class="mb-1.5 block text-[12px] font-medium text-white/55"
					>Nombre visible</label
				>
				<input
					id="caw-name"
					class="w-full rounded-[14px] border border-white/[0.14] bg-white/[0.07] px-4 py-3 text-[14px] text-white outline-none placeholder:text-white/25 focus:border-blue-500/65 focus:ring-[3px] focus:ring-blue-500/15"
					type="text"
				...
				<!-- Resumen compacto -->
				<div class="mt-4 rounded-2xl border border-white/[0.07] bg-white/[0.04] p-4">
					<p class="m-0 mb-3 text-[10px] font-bold uppercase tracking-widest text-white/30">
						Resumen
					</p>
					<dl class="m-0 space-y-2">
						{#each [['Tipo', wizard.type === 'ignition' ? 'Ignición' : 'Zona'], ['Condición', conditionLabel(wizard.condition)], ['Unidades', `${wizard.units?.length ?? 0} seleccionada${(wizard.units?.length ?? 0) !== 1 ? 's' : ''}`], ...(wizard.type === 'zone' && wizard.zone ? [['Zona', selectedZoneLabel]] : [])] as [dt, dd]}
							<div class="flex items-center justify-between gap-3">
								<dt class="m-0 text-[12px] text-white/45">{dt}</dt>
								<dd class="m-0 text-[12px] font-semibold">{dd}</dd>
							</div>
						{/each}
						<div class="flex items-center justify-between gap-3 border-t border-white/[0.07] pt-2">
							<dt class="m-0 text-[11px] text-white/35">Evento DB</dt>
		...
		<footer
			class="shrink-0 flex gap-3 border-t border-white/[0.07] bg-[#060b18] px-4 py-4 pb-[max(1rem,env(safe-area-inset-bottom))]"
		>
			{#if step > 1}
				<button
					type="button"
					class="h-12 flex-1 rounded-[14px] border border-white/[0.14] bg-white/[0.05] text-[14px] font-medium text-white/65 transition-colors hover:bg-white/[0.09] focus-visible:outline-2 focus-visible:outline-sky-400"
					on:click={handleBack}>Atrás</button
				>
			{/if}
			...
			{#if step < STEPS && !(wizard.type === 'ignition' && step === 3)}
				<button
					type="button"
					class="h-12 flex-[2] rounded-[14px] bg-gradient-to-br from-blue-600 to-cyan-600 text-[14px] font-bold text-white shadow-[0_4px_16px_rgba(37,99,235,0.4)] transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:bg-zinc-800 disabled:text-white/30 disabled:shadow-none focus-visible:outline-2 focus-visible:outline-sky-400"
					disabled={!canNext}
					on:click={handleNext}>Siguiente</button
				>
			{:else if step === 5 || (wizard.type === 'ignition' && step === 3)}
				{#if wizard.type === 'ignition' && step === 3}
					<button
						type="button"
						class="h-12 flex-[2] rounded-[14px] bg-gradient-to-br from-blue-600 to-cyan-600 text-[14px] font-bold text-white shadow-[0_4px_16px_rgba(37,99,235,0.4)] transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:bg-zinc-800 disabled:text-white/30 disabled:shadow-none"
						disabled={!canNext}
						on:click={handleNext}>Siguiente</button
					>
				{:else}
					<button
						type="button"
						class="h-12 flex-[2] rounded-[14px] bg-gradient-to-br from-emerald-600 to-teal-600 text-[14px] font-bold text-white shadow-[0_4px_16px_rgba(5,150,105,0.4)] transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:bg-zinc-800 disabled:text-white/30 disabled:shadow-none focus-visible:outline-2 focus-visible:outline-sky-400"
