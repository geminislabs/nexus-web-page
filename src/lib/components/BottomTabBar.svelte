...
<nav
	class="fixed bottom-0 left-0 right-0 z-[100] border-t border-white/[0.08] bg-slate-900 pb-[env(safe-area-inset-bottom,0px)] shadow-[0_-8px_24px_rgba(0,0,0,0.35)] sm:hidden"
		...
		{#each tabs as tab}
			...
			>
				<span class="relative flex h-[26px] w-[26px] items-center justify-center [&_.iconify]:h-[26px] [&_.iconify]:w-[26px]">
					...
					{#if tab.id === 'alertas' && $unreadAlarmCount > 0}
						<span
							class="absolute -right-2 -top-1 flex min-h-4 min-w-4 items-center justify-center rounded-full border-[1.5px] border-slate-900 bg-red-500 px-0.5 text-[9px] font-bold leading-none text-white"
							aria-hidden="true"
						>
