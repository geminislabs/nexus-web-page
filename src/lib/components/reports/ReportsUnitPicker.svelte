<script>
	...
	onDestroy(() => {
		if (browser && portalHost && portalHost.parentNode === document.body) {
...
{#if open}
	<div bind:this={portalHost}>
		<div
			class="fixed inset-0 z-[400] flex items-end justify-center bg-black/40 sm:items-center sm:p-4 dark:bg-black/50"
			role="presentation"
			on:click|self={onClose}
			on:keydown={(e) => e.key === 'Escape' && onClose()}
		>
			<div
				class="flex w-full max-w-md flex-col overflow-hidden rounded-t-2xl border border-slate-200 bg-white shadow-2xl dark:border-white/10 dark:bg-[#0c1829] sm:mb-0 sm:rounded-2xl"
				style="max-height: min(70vh, 480px); margin-bottom: max(0.75rem, env(safe-area-inset-bottom, 0px));"
				role="dialog"
				aria-modal="true"
			...
			>
						...
						{#each filtered as u (u.id)}
							...
							<li>
								...
								>
									...
									{#if selected}
										<span
											class="shrink-0 text-[11px] font-semibold text-cyan-700 dark:text-cyan-400"
											>✓</span
										>
									{/if}
								</button>
				...
				<div class="shrink-0 border-t border-slate-200 px-4 py-2.5 dark:border-white/[0.07]">
					<p class="m-0 text-center text-[11px] text-slate-500 dark:text-white/30">
						...
						{:else}
							Selecciona hasta {3 - selectedIds.size} unidad{3 - selectedIds.size !== 1 ? 'es' : ''}
							más
						{/if}
					</p>
				</div>
